...
ExcecaoCelulaInvalida::ExcecaoCelulaInvalida(int l, int c) {
    this->_linha = l;
    this->_coluna = c;
    this->_erro = "Célula (" + to_string(l) + ", " + to_string(c) + ") não é válida. Deseja continuar e ignorá-la? (s/n)?";
}

string ExcecaoCelulaInvalida::what() {
    return this->_erro;
}
